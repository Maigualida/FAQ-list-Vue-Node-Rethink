...
let connection = r.connect({
	...
	app.post('/newInfo', (req, res) => {
		let info = req.body;
		r.table('FAQ').insert(info).run(connection, (err, cursor) => {
			// setup email data with unicode symbols


			let mailOptions = {
				from: '"Mai" <mai@mail.com>', // sender address
				to: "n@gmail.com", // list of receivers
				subject: 'nueva preguta ✔', // Subject line
				text: 'algo de texto...', // plain text body
				html: `<h1>Hello!</h1>
	...
	app.get('/edit/:id', (req, res) => {
		...
		r.table('FAQ').get(id).run(connection, (err, result) => {
			res.json(result);
		})
	})
	...
	app.get('/search/:keyword', (req, res) => {
		...
		console.log(reg);
		r.table('FAQ').filter(r.row('question').match(keyword)).run(connection, (err, cursor) => {
			if (err) throw err;
			cursor.toArray((err, result) => {
				res.json(result);
			})
	...
	app.post('/editData', (req, res) => {
		...
		let rep = req.body.reponse;


		console.log(id, que, rep)
		r.table('FAQ').get(id).update({ reponse: rep, question: que }).run(connection, (err, cursor) => {
			//	if（err）throw err;
